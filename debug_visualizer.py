...
class DebugVisualizer:
    ...
    def update(self, event):
        ...
        # --- Run Logic ---
        self.left_hand.get_section_from_position()
        ...
        status_text_content = state_names.get(self.left_hand.glove_state, "Unknown State")

        if self.left_hand.glove_state == 2:
            status_text_content = status_text_content.format(
                x_sec=self.left_hand.x_section,
                y_sec=self.left_hand.y_section
            )
            status_text_content += f"\nPlane Extents: {self.left_hand.active_area_half_extents[:2]*2}"
            status_text_content += f"\nPlane Normal: Axis {self.left_hand.plane_normal_axis}"

        self.status_text.text = (
            f"Button: {'PRESSED' if self.left_hand.button_pressed else 'RELEASED'}\n"
            f"State: {status_text_content}\n"
            f"Controls: WASDQE move, IJKLUO rotate, SPACE draw/finalize, C clear (not implemented yet)"
        )
...
def main():
    ...
    glove_pair.left_hand.visual = jhk.GloveVisual(axis=scene.visuals.XYZAxis(parent=visualizer.view.scene), parent_scene=visualizer.view.scene)
